<template>
  <div class='px-4 py-20 bg-gray-100 sm:px-0'>
    ...
    <div v-else class='grid overflow-hidden gap-4 mx-auto max-w-7xl sm:px-6 lg:px-8 sm:grid-cols-2 md:grid-cols-3 sm:gap-8'>
      <div v-for='category in allCategoriesWithSubcategories.slice(0,1)' :key='category.id' class='relative p-6 bg-white rounded-lg group'>
        <div>
            <div>
      ...
      <div class="relative p-6 bg-white rounded-lg group">
            <div>
              <img src="https://admin.allsolutioncompany.com/images/allsolution.jpg"/>
            </div>
            <div class='mt-4'>
             
            </div>
      </div>
      <div v-for='category in allCategoriesWithSubcategories.slice(1,2)' :key='category.id' class='relative p-6 bg-white rounded-lg group'>
        <div>
            <div>
              <span :style="'background-color:'+category?.color" class='inline-flex p-2 rounded-lg ring-4 ring-white'>
                <img :alt='$filters.transString(category?.name)' :src='$filters.getFirstMediaUrl(category)' aria-hidden='true' class='w-12 h-12 opacity-70'>
