<template>
  <Disclosure v-slot='{ open }' as='nav' class='sticky text-white top-0 z-40 bg-allsolns-color-600 h-70'>
      ...
      <div class='flex justify-between h-16'>
        <div class='flex'>
          <div class='flex items-center mr-2 -ml-2 md:hidden'>
            <!-- Mobile menu button -->
            <DisclosureButton
            class='inline-flex justify-center items-center p-2 text-white rounded-md hover:text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-main-color-500'>
        ...
        <router-link as='div' class='flex flex-shrink-0 items-center cursor-pointer' to='/'>
          ...
          <span class='mx-3 font-bold text-white'>{{ this.$settings['app_name'] }}</span>
        </router-link>
        <div class='hidden md:ml-6 md:flex md:space-x-6 rtl:space-x-reverse'>
          <!-- <router-link
          :class="{'border-main-color-500 text-white font-bold': ($router.currentRoute.value.name === 'Home'),'border-transparent text-black-500 hover:border-gray-300 hover:text-gray-700': ($router.currentRoute.value.name !== 'Home')}"
...
<!--       <router-link
      :class="{'border-main-color-500 text-white font-bold': ($router.currentRoute.value.name === 'Help'),'border-transparent text-black-500 hover:border-gray-300 hover:text-gray-700': ($router.currentRoute.value.name !== 'Help')}"
      class='inline-flex items-center px-1 pt-1 text-sm font-medium border-b-2'
      to='/help'>
      {{ $t('Help & Support') }}
    </router-link> -->
...
<div class='flex gap-5 items-center'>
  ...
  class='hidden text-sm md:inline-flex focus:outline-none text-white hover:text-white'
  type='button'>
  {{ $t('Login') }}
</router-link>
...
<SearchBar v-if="is_searchComp"/>
</div>

<!-- Mobile menu -->
<DisclosurePanel class='md:hidden'>
  <div class='pb-3 bg-blue'>
    <router-link
